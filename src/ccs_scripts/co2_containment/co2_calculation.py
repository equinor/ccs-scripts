...
def _calculate_co2_data_from_source_data(
    ...
    if _is_subset(["SGAS"], active_props):
        ...
        if _is_subset(["PORV", "DGAS", "DWAT", "AMFG", "YMFG"], active_props):
            ...
            source = "Eclipse"
        elif any(prop in ["PORV", "DGAS", "DWAT", "AMFG", "YMFG"] for prop in active_props):
            missing_props = [x for x in ["PORV", "DGAS", "DWAT", "AMFG", "YMFG"] if x not in active_props]
        ...
        elif any(prop in ["RPORV", "BGAS", "BWAT", "XMF2", "YMF2"] for prop in active_props):
            missing_props = [x for x in ["RPORV", "BGAS", "BWAT", "XMF2", "YMF2"] if x not in active_props]
            error_text = "Lacking some required properties to compute CO2 mass/volume"
            error_text += "\nMissing: "
            error_text += ", ".join([property for property in missing_props])
        else:
            error_text = "None of the required properties to compute CO2 mass/volume are available"
            raise RuntimeError(error_text)
